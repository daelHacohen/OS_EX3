...
void* monitor_thread(void* arg) {
    ...
    while (1) {
        ...
        pthread_cond_wait(&cond, &pthread_lock);

        bool current_status = CH >= 100;
...
int main() {
    ...
    // }
    Proactor proactor;
    proactor.startProactor(server_fd, handle);
    pause();
    ...
    while (true) {
        ...
        if (before_space == "CH") {
            ...
            if(area>=100){
                 ...
                 pthread_mutex_lock(&pthread_lock);
                 print=true;
                 pthread_cond_signal(&cond); // מעיר את ה-thread רק אם יש שינוי
                 pthread_mutex_unlock(&pthread_lock);
            }
            if (area<100 && gotCH)
            {
